...
export class Chart {
    ...
    render() {
        ...
        this.dotSvgChart = this.container.append('svg')
            .attr('width', outerWidth)
            .attr('height', outerHeight)
            .style('z-index', 0)
            .attr('class', 'svg-plot')
        ...
        this.dot = this.dotSvgChart.append('g')
            .selectAll('circle')
            .data(this.data)
            .enter().append('circle')
            // .join('circle')
        ...
        this.zoom_function = d3.zoom().scaleExtent([1, 1000])
            .on('zoom', () => {
                ...
                this.lastXdomain = scaleX.domain();
                this.lastYdomain = scaleY.domain();

                console.log('draw')
                this.dot.attr('transform', this.lastTransform);


            });

        this.dotSvgChart.call(this.zoom_function);
    ...
    draw (transform, groupId) {
        ...
        function isBrushed(point) {
            if (point[0] >= this.lastXdomain[0]  && point[0] <= this.lastXdomain[1] &&
                point[1] >= this.lastYdomain[0]  && point[1] <= this.lastYdomain[1]) {
                return true;
            }
            return false;
        }
    }
    drawPoint (scaleX, scaleY, point, k, groupId) {
        // this.context.beginPath();
        // this.context.fillStyle = groupId == point[2] ? 'red' : pointColor[Math.floor(Math.random() * 10 % 4)]  ;
        // const px = scaleX(point[0]);
        // const py = scaleY(point[1]);
        //
        // this.context.arc(px, py, (groupId == point[2] ? 1.2*2 : 1.2) * k, 0, 2 * Math.PI, true);
        // this.context.fill();
    }
    resetZoom () {
    ...
    handleBrushEnd () {
        ...
        console.log("brushCode", d3.brushSelection(this));
